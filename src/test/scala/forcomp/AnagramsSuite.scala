...
class AnagramsSuite extends munit.FunSuite:
  ...
  test("subtract") {
    val originalOccurrences = List(('a', 3), ('b', 2), ('c', 1), ('d', 1))
    val occurrencesToRemove = List(('a', 3), ('b', 1), ('c', 1))
    val expectedNewOccurrences = List(('b', 1), ('d', 1))
    val actualNewOccurrences = subtract(originalOccurrences, occurrencesToRemove)
    assertEquals(actualNewOccurrences, expectedNewOccurrences)
  }
  ...
  test("sentence anagrams yes man") {
    ...
    val subsets = combinations(sentenceOccurrences(sentence))
    val expected = List(
      List("en", "as", "my"),
      List("en", "my", "as"),
      List("man", "yes"),
    ...
    assertEquals(sentenceAnagrams(sentence).toSet, expected.toSet)
  }
